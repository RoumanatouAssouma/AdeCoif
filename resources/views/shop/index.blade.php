            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
                ...
                <div class="w-full md:w-48">
                    <form action="{{ route('shop') }}" method="GET">
                        ...
                        @endif
                        <select name="sort" onchange="this.form.submit()" class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-rose-500 focus:border-rose-500">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                ...
                <div class="bg-white rounded-lg shadow-md overflow-hidden group">
                    ...
                    <div class="p-4">
                        ...
                        <p class="font-bold text-rose-600 mb-4">{{ $product->formatted_price }}</p>
                        <div class="flex gap-2">
                            <a href="{{ route('product.show', $product->slug) }}" class="flex-1 inline-flex items-center justify-center px-4 py-2 bg-rose-600 text-white rounded-md hover:bg-rose-700 text-sm font-medium">
                                Voir détails
                            </a>
                            <form action="{{ route('cart.add') }}" method="POST">
                                @csrf
                                <input type="hidden" name="product_id" value="{{ $product->id }}">
                                <button type="submit" class="inline-flex items-center justify-center px-3 py-2 border border-rose-600 text-rose-600 rounded-md hover:bg-rose-50 text-sm font-medium">
